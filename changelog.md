...
- NeoForge: The Mods screen config button is now enabled again.
- Forge: The Mods screen config button now opens the Konfig config screen.

## 4.1.0

### Changed

- The Key Mode client option now uses a dropdown in the in-game config screen.
- Liteminer now requires Konfig 0.5.0 or newer.
...
### Added
- Synced French, Brazilian Portuguese, Simplified Chinese, and Traditional Chinese translations from `i18n.kaf.sh` across the supported versions.

### Credits
- French translation by `.Polloclown`.
- Brazilian Portuguese translation by `Kaf`.
- Simplified Chinese translation by `虾米逃生计划`.
- Traditional Chinese translation by `sunmoonzero`.

## 3.0.2

### Fixed
- Staircase shapes now follow the mined block face instead of coarse player yaw.
- Staircase Up now validates the actual lower block before mining it.
- Staircases now stop exactly at the configured block break limit.

### Changed
- Staircase Up and Staircase Down now mine a three-block-tall path.

### Credits

- Staircase-down adjustment by Evarika.

## 3.0.0

### Added
- Ported Liteminer to Minecraft **26.1**.

### Changed
- Liteminer `26.1` now targets Java **25**.
- This release is available on **Fabric** and **NeoForge**.
