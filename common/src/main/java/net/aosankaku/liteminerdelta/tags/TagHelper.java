...
public class TagHelper {
    ...
    public static boolean isExcludedBlock(BlockState block) {
        return isOr(block, LiteminerTags.Blocks.EXCLUDED_BLOCKS, LiteminerTags.Legacy.EXCLUDED_BLOCKS,
                LiteminerTags.Compat.EXCLUDED_BLOCKS);
    }

    public static boolean isWhitelistedBlock(BlockState block) {
        return isOr(block, LiteminerTags.Blocks.BLOCK_WHITELIST, LiteminerTags.Legacy.BLOCK_WHITELIST,
                LiteminerTags.Compat.BLOCK_WHITELIST);
    }

    public static boolean isBlockWhitelistEnabled() {
        return isBlockTagPopulated(LiteminerTags.Blocks.BLOCK_WHITELIST)
                || isBlockTagPopulated(LiteminerTags.Legacy.BLOCK_WHITELIST)
                || isBlockTagPopulated(LiteminerTags.Compat.BLOCK_WHITELIST);
    }

    public static boolean isExcludedTool(ItemStack stack) {
        return isOr(stack, LiteminerTags.Items.EXCLUDED_TOOLS, LiteminerTags.Legacy.EXCLUDED_TOOLS,
                LiteminerTags.Compat.EXCLUDED_TOOLS);
    }

    public static boolean isIncludedTool(ItemStack stack) {
        return isOr(stack, LiteminerTags.Items.INCLUDED_TOOLS, LiteminerTags.Legacy.INCLUDED_TOOLS,
                LiteminerTags.Compat.INCLUDED_TOOLS);
    }
    ...
    public static void report(ServerLevel serverLevel) {
        ...
        initialized = true;
        Liteminer.LOGGER.info("Loading Liteminer Delta Tags...");
