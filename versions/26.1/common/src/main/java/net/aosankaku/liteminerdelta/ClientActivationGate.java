...
final class ClientActivationGate {
    ...
    Result evaluate(boolean requestedByKeybind, boolean serverSupported) {
        if (serverSupported) { unavailableNotificationShown = false; return Result.ALLOW; }
    ...
    void reset() { unavailableNotificationShown = false; }
    enum Result { ALLOW, BLOCK, BLOCK_AND_NOTIFY }
}
